...
from tensorflow.keras.layers import Input, Flatten, Dense, Conv2D, Concatenate, Dropout, BatchNormalization, MaxPooling2D
...
def efficientnet_block(x, filters, kernel_size=3, strides=1):
...
# EfficientNet-like blocks with Dropout
x = efficientnet_block(input_layer, 8)
x = Dropout(0.2)(x)  

x = efficientnet_block(x, 16)
x = Dropout(0.2)(x)  

x = efficientnet_block(x, 32, strides=2)  # Downsample
x = Dropout(0.2)(x)  

x = efficientnet_block(x, 64, strides=2)  # Downsample
x = Dropout(0.2)(x)  
